/*
 *   CGameMainBA.cpp   : ゲーム本体(BossAttack)
 *
 */
...
#include "RndCtrl.h"
	...
	BOOL CGameMainBA::m_Debug_DisplayInfo = FALSE;	// 表示するか否か
	BYTE CGameMainBA::m_Debug_EffectCount = 0;		// 表示フェードエフェクト用カウンタ
#endif



// コンストラクタ(矩形は非x２５６) //
CGameMainBA::CGameMainBA(RECT *rcTarget)
{
	int		*pX, *pY;
	DWORD	TexID;
...
//		if(NULL == m_pAtkGrpDraw) break;
		...
		// ＳＣＬ(for BossAttack)デコードクラスを生成する //
		m_pSCLDecode = NewEx(CSCLBADecoder(&m_rcTargetX256, m_pEnemyCtrl));
...
// 初期化を行う //
FBOOL CGameMainBA::Initialize(
						CHARACTER_ID	CharID,	// 自分のキャラクタＩＤ
						DWORD			Seed)	// 乱数のたね
{
	// 乱数系列の初期化 //
	m_Rnd.SetSeed(Seed);
...
//PbgLog(" SCLDec");
	...
	// ＳＣＬデコーダを初期化 //
	if(FALSE == m_pSCLDecode->Initialize(&m_Rnd)) return FALSE;
//PbgLog(" SCLDec->Ini");
	...
	if(FALSE == m_pPlayerCtrl->PlayerInitialize(
								CharID						// キャラクタＩＤ
							  ,	1							// 初期攻撃レベル
							  , NULL//pRival->m_pStdAttack		// 標準攻撃送り
							  , NULL//pRival->m_pSCLDecode		// ＳＣＬデコーダ
							  , NULL//pRival->m_pBGDraw			// 背景描画
							  , NULL//pRival->m_pAtkGrpDraw		// キャラ描画
							  , m_pBGDraw					// 背景描画
							  , m_pEnemyCtrl				// 敵管理
							  , NULL//pRival->m_pEnemyCtrl		// 相手側の敵管理
							  , CGAUGE_LEVEL1
							  , NULL	//m_pStateWindow
							  , NULL	//pRival->m_pStateWindow
							  )){
		return FALSE;
	}
//PbgLog(" PLCtrl->Ini");
...
//PbgLog("BA Initialzied!!");
	return TRUE;
}


// １フレーム進める //
BOOL CGameMainBA::Move(WORD KeyCode)
{
	DWORD		Shave;
	DWORD		Damage;
	DWORD		Score;
...
*/
		// 取得した値を実際に反映する //
		if(Damage) m_pPlayerCtrl->OnDamage(99999999);
...
*/
	return TRUE;//GMAIN_OK;
}


// 描画を行う //
FBOOL CGameMainBA::Draw(int SyncDx)
{
	// 描画対象となる矩形をセットする //
...
// その他情報の描画を行う //
FBOOL CGameMainBA::DrawInfo(Pbg::CGrpSurface *pSurface)
{
...
FBOOL CGameMainBA::IsGameFinished(void)
{
...
FVOID CGameMainBA::OnETClear(BOOL bChgScore)
{
	// 消去は敵管理クラスに委譲する //
	m_pScore->AddScore(
...
// 現在のスコアを取得する //
__int64 CGameMainBA::GetCurrentScore(void)
{
...
// ヒットチェック用コールバック関数 //
DWORD CGameMainBA::HitCheckCallback(void *pParam, EnemyData *pEnemy)
{
	CGameMainBA		*pThis;

	pThis = (CGameMainBA *)pParam;
...
// 動的確保したオブジェクトの解放を行う //
FVOID CGameMainBA::Cleanup(void)
{
